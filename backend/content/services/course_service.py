...
def _build_queryset(filters: CourseFilter, strategy: CourseFetchStrategy):
    ...
    if filters.enrolled_user:
        ...
        query_set = query_set.filter(enrollments__user=filters.enrolled_user)
        ...
        query_set = query_set.annotate(
            ...
            user_cached_completed_lessons=F('enrollments__cached_completed_lessons'),
            user_cached_total_lessons=F('enrollments__cached_total_lessons'),
        )
    ...
    # 2. STRUCTURE / DETAIL (Màn hình học/xem chi tiết)
        ...
        query_set = query_set.prefetch_related(
            ...
            Prefetch(
                'modules__lessons__content_blocks',
                queryset=ContentBlock.objects.order_by('position').only('id', 'title', 'type', 'lesson_id', 'payload')
            )
        )
        ...
        query_set = query_set.annotate(
            modules_count=Count('modules', distinct=True),
            total_lessons=Count('modules__lessons', distinct=True),
            # total_seconds=... (Nếu cần hiển thị "Tổng thời lượng khóa học: 10h")
        )
