...
#     # # 3. Duyệt từng câu hỏi để phân tích
#     # questions = quiz.questions.all().order_by('position')
#     # question_reports = []

#     # for q in questions:
#     #     # Lấy câu trả lời cho câu hỏi này
#     #     q_answers = QuestionAnswer.objects.filter(question=q, attempt__status='graded')
#     #     q_total = q_answers.count()
        
#     #     if q_total == 0: 
#     #         continue

#     #     # A. Difficulty Index (p): Tỷ lệ làm đúng
#     #     correct_count = q_answers.filter(is_correct=True).count()
#     #     difficulty_index = (correct_count / q_total) * 100

#     #     # B. Discrimination Index (D): (P_top - P_bottom)
#     #     # P_top: Tỷ lệ nhóm giỏi làm đúng
#     #     # P_bottom: Tỷ lệ nhóm yếu làm đúng
        
#     #     top_correct = q_answers.filter(attempt_id__in=top_group_ids, is_correct=True).count()
#     #     bottom_correct = q_answers.filter(attempt_id__in=bottom_group_ids, is_correct=True).count()
        
#     #     # Tránh chia cho 0
#     #     p_top = top_correct / len(top_group_ids) if top_group_ids else 0
#     #     p_bottom = bottom_correct / len(bottom_group_ids) if bottom_group_ids else 0
        
#     #     discrimination_index = round(p_top - p_bottom, 2)

#     #     # C. Đánh giá & Khuyến nghị (Rule-based AI)
#     #     status, recommendation = _evaluate_question(difficulty_index, discrimination_index)

#     #     # D. Distractor Analysis (Phân tích đáp án nhiễu)
#     #     # Chỉ áp dụng cho trắc nghiệm (Multiple Choice)
#     #     dist_data = {}
#     #     if q.type in ['multiple_choice_single', 'multiple_choice_multi']:
#     #         dist_data = _analyze_distractors(q, q_answers)

#     # # 4. Tính chỉ số tổng quan Quiz
#     # avg_score = attempts.aggregate(Avg('score'))['score__avg'] or 0

#     # # SAU KHI TÍNH TOÁN XONG HẾT CHO TẤT CẢ CÂU HỎI:
...
#     #     total_attempts=total_attempts_count,
#     #     average_score=round(avg_score, 2),
#     #     # standard_deviation=... (nếu tính thêm)
#     # )

#     # domain_questions = []

#     # # B. Lưu Question Snapshots (Bulk Create cho nhanh)
#     # quest_stat_objs = []

#     # for q_data in question_reports: # Giả sử bạn gom data vào list này
#     #     # Tạo Model Object
#     #     quest_stat_objs.append(QuestionStatisticSnapshot(
#     #         quiz_stat=quiz_snapshot,
#     #         question_id=q_data['question_id'],
#     #         prompt_text_snapshot=q_data['prompt_text'],
#     #         total_attempts=q_data['total_attempts'],
#     #         correct_ratio=q_data['correct_ratio'],
#     #         discrimination_index=q_data['discrimination_index'],
#     #         option_distribution=q_data['dist_data'],
#     #         status=q_data['status'],
#     #         recommendation=q_data['recommendation']
#     #     ))

#     #     question_reports.append(QuestionAnalysisDomain(
#     #         question_id=str(q.id),
#     #         prompt_text=q.prompt.get('text', 'No text')[:80],
#     #         question_type=q.type,
#     #         total_attempts=q_total,
