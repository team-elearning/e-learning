            ...
            try:
                if isinstance(instance_data, (list, QuerySet)):
                    ...
                    for item in instance_data:
                        ...
                        elif hasattr(item, "__dict__"):  # Plain object
                            data = item.__dict__
                        else:
                            raise ValueError(f"Cannot convert item of type {type(item)} to dict")
            
                        # Validate + create DTO
                        dto_instance = dto_cls.model_validate(data)
                        # Serialize to JSON and add to list
                        serialized_list.append(dto_instance.model_dump())

                    # Serialize to JSON
                    response.data = serialized_list
                
                else:
                    domain_obj = instance_data # A single object
                    
                    # Convert domain_obj to dict
                    if hasattr(domain_obj, "model_dump"):  # Pydantic model
                        data = domain_obj.model_dump()
                    elif hasattr(domain_obj, "__dict__"):  # Plain object
