...
from quiz.models import UserAnswer, Question, QuizAttempt
from quiz.services.quiz_user_service import save_answer, start_or_resume_attempt, submit_attempt
...
class StudentQuizAttemptStartView(APIView):
    ...
    def post(self, request, pk):
        """
        Bắt đầu làm bài hoặc Resume.
        """
        try:
            attempt, action = start_or_resume_attempt(pk, request.user)
            
            # Trả về ID attempt để FE redirect sang trang làm bài
            return Response({
                "attempt_id": attempt.id,
                "action": action, # 'created' hoặc 'resumed'
