...
class BlockProgressPublicOutput(BaseModel):
    model_config = ConfigDict(from_attributes=True)
...
class BlockProgressAdminOutput(BaseModel):
    model_config = ConfigDict(from_attributes=True)

    block_id: UUID # Cần ID để admin thực hiện thao tác sửa/xóa nếu cần
...
class CourseProgressPublicOutput(BaseModel):
    model_config = ConfigDict(from_attributes=True)
