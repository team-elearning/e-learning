...
export type TxStatus = 'Pending' | 'Processing' | 'Succeeded' | 'Failed' | 'Refunded' | 'Disputed'
...
// Backend payment history response
export interface PaymentHistoryItem {
  ...
  status_variant: 'pending' | 'success' | 'failed' | 'refunded'
  created_at: string
  paid_at: string | null
}
...
// Helper to convert backend payment history to TxSummary
function convertPaymentHistoryToTxSummary(item: PaymentHistoryItem): TxSummary {
  ...
  return {
    id: item.id,
    userId: 0, // Backend doesn't return user info in list
    buyerName: '',
    buyerEmail: '',
    courseId: 0,
    courseTitle: item.plan_name,
    amount: item.amount,
    currency: 'VND',
    gateway: item.method as Gateway,
    status: statusMap[item.status] || 'Pending',
    createdAt: item.created_at,
    settledAt: item.paid_at || undefined,
  }
}
...
export const paymentService = {
  ...
  async detail(id: ID): Promise<TxDetail> {
    ...
    const events: TxEvent[] = [
      ...
      {
        time: new Date(base + 5 * 60 * 1000).toISOString(),
        type: 'authorized',
        description: 'Uỷ quyền thanh toán',
      },
    ]
