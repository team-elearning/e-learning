...
export const courseService = {
    ...
    async list(params: PageParams): Promise<PageResult<CourseSummary>> {
        ...
        if (params.q) {
            ...
            list = list.filter(c => c.title.toLowerCase().includes(q) || String(c.id).includes(q) || c.teacherName.toLowerCase().includes(q))
        }
    ...
    async detail(id: ID): Promise<CourseDetail> {
        ...
        return {
            ...
            status: (['draft', 'pending_review', 'published', 'rejected', 'archived'] as CourseStatus[])[Number(id) % 5],
            ...
            durationMinutes: sections.reduce((a, s) => a + s.lessons.reduce((b, l) => b + (l.durationMinutes || 0), 0), 0),
            sections
        }
    },
