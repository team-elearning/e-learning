...
export const userService = {
  ...
  update(
    id: ID,
    payload: { username: string; email: string; phone?: string | null; name?: string },
  ) {
  ...
  revokeSession(id: ID, sid: string) {
    return api.delete(`/account/admin/users/${id}/sessions/${sid}/`)
  },
  revokeAll(id: ID) {
    return api.delete(`/account/admin/users/${id}/sessions/`)
  },
  ...
  async loginHistory(
    ...
    if (!USE_MOCK) {
      ...
      if (Array.isArray(data.results))
        return { items: data.results, total: data.count ?? data.total ?? data.results.length }
  ...
  async transactionsByUser(
    ...
    const items: Transaction[] = Array.from({ length: size }).map((_, i) => {
      ...
      return {
        ...
        status: (['Succeeded', 'Refunded', 'Failed', 'Processing'] as Transaction['status'][])[
          idx % 4
        ],
  ...
  async listNotes(id: ID): Promise<NoteItem[]> {
    if (!USE_MOCK) {
      const { data } = await api.get(`/account/admin/users/${id}/notes/`)
      return data
    }
    return [
      {
        id: 'n1',
        author: 'Admin',
        note: 'HS học chăm, tiến bộ tốt.',
        time: new Date(Date.now() - 864e5).toISOString(),
      },
      {
        id: 'n2',
        author: 'QA',
        note: 'Yêu cầu xác minh số điện thoại.',
        time: new Date(Date.now() - 2 * 864e5).toISOString(),
      },
    ]
  },
  async addNote(id: ID, note: string) {
    if (!USE_MOCK) {
      return api.post(`/account/admin/users/${id}/notes/`, { note })
    }
    return { ok: true }
  },
  ...
  bulkChangeRole(ids: ID[], role: Role) {
    return api.post('/account/admin/users/bulk/role/', { ids, role })
  },
  bulkLock(ids: ID[]) {
    return api.post('/account/admin/users/bulk/lock/', { ids })
  },
  bulkUnlock(ids: ID[]) {
    return api.post('/account/admin/users/bulk/unlock/', { ids })
  },
  bulkBan(ids: ID[]) {
    return api.post('/account/admin/users/bulk/ban/', { ids })
  },

  async exportCsv(params: PageParams): Promise<Blob> {
    if (!USE_MOCK) {
      const { data } = await api.get('/account/admin/users/export/', {
        params,
        responseType: 'blob',
      })
      return data
