<template>
  <nav class="sticky top-0 z-50 h-16 bg-white/90 backdrop-blur-lg border-b border-gray-200/80 shadow-sm">
    <div class="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <!-- Right Actions -->
      <div class="flex items-center gap-3">
        <!-- 🔔 Notification Button: hover luôn có; màu & badge chỉ khi có thông báo -->
        <button
        ...
        >
          ...
          <!-- Badge & ping: chỉ render khi có thông báo -->
...
/** 🔔 Demo: số thông báo chưa đọc (thay bằng store của bạn nếu có) */
const unreadCount = ref(0) // ví dụ: computed(() => useNotificationStore().unreadCount)
...
.logo-glow-bg {
  ...
  background: radial-gradient(circle, rgba(59,130,246,0.3) 0%, rgba(20,184,166,0.2) 30%, transparent 70%);
  border-radius: 50%; opacity: 0; filter: blur(20px);
  transition: opacity 0.5s ease, transform 0.5s ease; animation: float 3s ease-in-out infinite;
}
...
/* ===== Gradient Animation for Bottom Border ===== */
...
.ripple-effect {
  ...
  animation: ripple-out 0.8s cubic-bezier(0.34, 1.56, 0.64, 1); pointer-events: none;
}
...
.notification-ping {
  position: absolute; height: 10px; width: 10px; border-radius: 50%;
  background: linear-gradient(135deg, #ef4444, #f97316);
  animation: ping-animation 1.5s cubic-bezier(0, 0, 0.2, 1) infinite;
}
.notification-dot {
  position: relative; height: 8px; width: 8px; border-radius: 50%;
  background: linear-gradient(135deg, #ef4444, #f97316);
...
.avatar-glow-ring {
  ...
  background: linear-gradient(135deg, rgba(16,185,129,0.5), rgba(59,130,246,0.5), rgba(16,185,129,0.5));
  background-size: 200% 200%; opacity: 0; filter: blur(8px); transition: opacity 0.4s ease;
...
.online-pulse {
  position:absolute; height:12px; width:12px; border-radius:50%;
  background: linear-gradient(135deg, #10b981, #34d399);
  animation: pulse-animation 2s cubic-bezier(0.4,0,0.6,1) infinite;
}
.online-dot {
  position:relative; height:10px; width:10px; border-radius:50%;
  background: linear-gradient(135deg, #10b981, #34d399);
  border:2px solid white; box-shadow:0 0 0 1px rgba(16,185,129,0.2), 0 2px 4px rgba(0,0,0,0.2);
}
...
.status-badge {
  ...
  background: linear-gradient(135deg, #d1fae5, #a7f3d0); font-size:10px; font-weight:600; color:#059669;
}
...
.menu-item::before {
  ...
  transition: transform .3s cubic-bezier(0.34,1.56,0.64,1); border-radius:0 3px 3px 0;
}
...
.hamburger-icon span {
  display:block; width:20px; height:2px; background: linear-gradient(90deg, #6b7280, #374151);
  border-radius:2px; transition:all .3s cubic-bezier(0.34,1.56,0.64,1); transform-origin:center;
}
