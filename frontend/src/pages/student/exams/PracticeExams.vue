      ...
      <div v-else-if="exams.length > 0" class="exams-grid">
        <article v-for="e in exams" :key="e.id" class="exam-card">
          ...
          <div class="card-footer">
            ...
            <router-link
              class="start-btn"
              :to="{ name: 'student-practice-taking', params: { examId: e.id } }"
            >
              Bắt đầu
              <svg
                xmlns="http://www.w3.org/2000/svg"
...
// --- Helpers ---
function qCount(e: any) {
  return (e?.questionsCount ?? e?.questions ?? 0) as number
}
...
.select-btn[aria-expanded='true'] .select-icon {
