      ...
      <main v-else class="exam-main">
        ...
        <div class="question-card-wrapper">
          ...
          <div class="card-footer">
            <button class="btn btn-secondary" :disabled="idx === 0" @click="prev">
              ‹ Câu trước
            </button>
            <div v-if="idx === questions.length - 1 && questions.length > 0" class="submit-area">
              <span class="answered-count"
                >{{ answeredCount }}/{{ questions.length }} câu đã trả lời</span
              >
      ...
      <div v-if="showSubmitModal" class="modal-backdrop" @click.self="showSubmitModal = false">
        <div class="modal-card" role="dialog" aria-modal="true">
          <header class="modal-header">
            ...
            <button class="modal-close" aria-label="Đóng" @click="showSubmitModal = false">
              ×
            </button>
          </header>
...
.opt-label:has(input:checked) .opt-key {
  background-color: #16a34a;
  color: #fff;
  border-color: #16a34a;
}
