    ...
    <div class="container">
      ...
      <section class="card hero">
        ...
        <div class="hero-body" v-if="resumeCourse">
          ...
          <div class="hero-progress">
            <span>Progress</span><div class="spacer" />
      ...
      <section class="card">
        ...
        <div class="courses" v-if="featured.length">
          <div class="course-card" v-for="c in featured" :key="String(c.id)" @click="openCourse(Number(c.id))">
            <img class="thumb" :src="c.thumbnail" :alt="c.title" />
            <div class="title">{{ c.title }}</div>
            <div class="progress-line"><div class="bar" :style="{ width: ((c.done ? 100 : c.progress) || 0) + '%' }"></div></div>
            <div class="status muted">{{ c.done ? 'Đã hoàn thành' : `Đang học · ${c.progress}%` }}</div>
          </div>
        </div>
        ...
        <ul class="exams" v-if="previewExams.length">
          <li class="exam-row" v-for="e in previewExams" :key="String(e.id)">
            ...
            <div class="exam-main">
              ...
              <div class="muted small">Khối {{ e.grade }} · {{ toMin(e.duration) }} phút · Đạt ≥ {{ e.pass }} câu</div>
            </div>
...
async function fetchCourses() {
  try {
    const { items } = await courseService.list({
      page: 1, pageSize: 12, status: 'published', sortBy: 'updatedAt', sortDir: 'descending',
    })
  ...
  } catch (e: any) { errMsg.value = `courseService.list lỗi: ${e?.message || String(e)}` }
}
...
function hasRoute(name: string) { return router.hasRoute(name as any) }
...
onMounted(async () => {
  await fetchCourses()
  try { await examStore.fetchExams() } catch (e:any) { errMsg.value = `examStore.fetchExams lỗi: ${e?.message || String(e)}` }
})
</script>

<style scoped>
:root{--bg:#f6f7fb;--card:#fff;--text:#0f172a;--muted:#6b7280;--line:#e5e7eb;--accent:#2563eb}
.student-dashboard{min-height:100vh;background:var(--bg);color:var(--text)}
.container{padding:20px 16px 40px;max-width:980px;margin:0 auto;display:grid;gap:16px}
.card{background:var(--card);border:1px solid var(--line);border-radius:14px;box-shadow:0 1px 0 rgba(16,24,40,.02);padding:14px}
.err{border:1px solid #fecaca;background:#fff1f2;color:#7f1d1d;border-radius:12px;padding:12px}
.err-title{font-weight:700;margin-bottom:6px}
.err-pre{white-space:pre-wrap;font-family:ui-monospace,Menlo,monospace;font-size:12px}
.hero .hero-head{font-weight:600;font-size:20px;margin-bottom:12px}
.hero-body{background:#0b2952;color:#e6eefc;border-radius:12px;padding:16px}
.hero-title{font-size:18px;font-weight:700}
.hero-sub{margin-top:6px;opacity:.9}
.hero-progress{margin-top:14px;display:flex;align-items:center}
.hero-progress .spacer{flex:1}
.btn-primary{display:inline-flex;align-items:center;justify-content:center;min-width:110px;height:44px;padding:0 16px;border-radius:10px;background:#fff!important;color:#0b2952!important;border:1px solid #c9d9ff!important;font-weight:700;font-size:14px}
.btn-primary:hover{background:#f6f9ff!important;border-color:#b7cbff!important}
.section-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:8px}
.section-head h2{font-size:16px;font-weight:700}
.ghost{background:transparent;border:none;font-size:20px;line-height:1;cursor:pointer}
.courses{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}
.course-card{border:1px solid var(--line);border-radius:12px;padding:10px;cursor:pointer;background:#fff}
.course-card:hover{background:#fafafa}
.course-card .thumb{width:100%;height:90px;object-fit:cover;border-radius:8px;margin-bottom:8px}
.course-card .title{font-weight:600}
.progress-line{margin-top:8px;height:6px;background:#eef2f7;border-radius:999px;overflow:hidden}
.progress-line .bar{height:100%;background:var(--accent)}
.status{margin-top:6px;font-size:12px}
.exams{display:grid;gap:10px}
.exam-row{display:grid;grid-template-columns:28px 1fr auto;gap:10px;align-items:center;border:1px solid var(--line);border-radius:12px;padding:10px}
.checkbox{position:relative;width:18px;height:18px;display:grid;place-items:center}
.checkbox input{position:absolute;inset:0;opacity:0;cursor:default}
.checkbox span{width:16px;height:16px;border:1px solid var(--line);border-radius:4px;background:#fff}
.exam-title{font-weight:700}
.btn-outline{background:transparent;border:1px solid var(--line);padding:8px 12px;border-radius:10px;cursor:pointer;font-weight:700}
.muted{color:var(--muted)}
.small{font-size:12px}
@media (max-width:900px){.courses{grid-template-columns:repeat(2,1fr)}}
