        ...
        <table class="w-full table-fixed">
          ...
          <tbody>
            <tr v-for="i in 5" :key="'skel-'+i" class="border-t">
              ...
              <td class="p-3"><div class="h-5 w-24 rounded-full bg-slate-100 animate-pulse"></div></td>
              <td class="p-3">
            ...
            <tr v-for="a in assignments" :key="a.id" class="border-t text-sm">
              ...
              <td class="p-3">
                <span
                  ...
                  :class="a.published ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'"
                >
              ...
              <td class="p-3">
                <div class="flex flex-wrap gap-2">
                  <button class="rounded-xl border px-3 py-1.5 text-sm hover:bg-slate-50" @click="viewAssignment(a.id)">Xem</button>
                  <button class="rounded-xl border px-3 py-1.5 text-sm hover:bg-slate-50" @click="gradeAssignment(a.id)">Chấm điểm</button>
                  <button class="rounded-xl bg-sky-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-sky-700" @click="editAssignment(a.id)">Sửa</button>
                </div>
              </td>
            </tr>
          </tbody>
 ...
 * Nếu bạn có file '@/services/class.service' với:
 *  - classService.detail(id): Promise<{id,name}>
 *  - classService.listAssignments({classId,page,pageSize}): Promise<{items, total}>
 * Thì code sẽ tự dùng. Không có → dùng mock.
 */
let fetchDetailFn: undefined | ((id:number)=>Promise<ClassDetail>)
let fetchAssignmentsFn: undefined | ((p:{classId:number; page:number; pageSize:number})=>Promise<{items:Assignment[]; total:number}>)
...
try {
  ...
  if (mod?.classService?.detail) {
    fetchDetailFn = mod.classService.detail as (id:number)=>Promise<ClassDetail>
  }
  if (mod?.classService?.listAssignments) {
    fetchAssignmentsFn = mod.classService.listAssignments as (p:{classId:number; page:number; pageSize:number})=>Promise<{items:Assignment[]; total:number}>
  }
...
function mockAssignments(params:{classId:number; page:number; pageSize:number}): Promise<{items:Assignment[]; total:number}> {
...
async function fetchList(p = page.value) {
  ...
  try {
    const res = fetchAssignmentsFn
      ? await fetchAssignmentsFn({ classId: classId.value, page: page.value, pageSize: pageSize.value })
      : await mockAssignments({ classId: classId.value, page: page.value, pageSize: pageSize.value })
...
const pagesToShow = computed(() => {
  const max = totalPages.value, cur = page.value, windowSize = 7
  ...
  if (max <= windowSize + 2) { for (let i=1;i<=max;i++) push(i) }
    ...
    push(1)
    const start = Math.max(2, cur-2), end = Math.min(max-1, cur+2)
    if (start > 2) sep('s')
...
/** ===== Actions (demo) ===== */
function createAssignment() { alert('Tạo bài tập (demo). Gắn router/Modal của bạn ở đây.') }
function viewAssignment(id:number) { alert(`Xem bài tập #${id} (demo)`) }
function gradeAssignment(id:number) { alert(`Chấm điểm bài tập #${id} (demo)`) }
function editAssignment(id:number) { alert(`Sửa bài tập #${id} (demo)`) }
