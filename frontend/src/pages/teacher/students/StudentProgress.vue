  ...
  <div class="min-h-screen w-full overflow-x-hidden bg-slate-50">
    <main class="w-full mx-auto max-w-screen-2xl px-6 py-8 md:px-10">
      ...
      <div class="mb-5 grid grid-cols-1 gap-3 md:grid-cols-4">
        ...
        <!-- Status -->
        <select v-model="status" class="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm">
        ...
        <!-- Sort -->
        <select v-model="sortBy" class="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm">
      ...
      <div v-else class="grid grid-cols-1 gap-3">
        ...
        >
          <img :src="u.avatar || fallbackAvatar(u)" :alt="u.name" class="h-12 w-12 rounded-full object-cover" />
          ...
          <div class="flex shrink-0 gap-2">
            <button class="rounded-xl border px-3 py-2 text-sm hover:bg-slate-50" @click="openFeedback(u.id)">
              Phản hồi
            </button>
          </div>
        </article>

        <p v-if="!list.length" class="mt-10 text-center text-slate-500">Không có học sinh phù hợp.</p>
      </div>
      ...
      <div class="mt-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <select v-model.number="page" class="rounded-xl border px-2 py-2 text-sm" @change="go(page)">
          ...
          <select v-model.number="pageSize" class="ml-2 rounded-xl border px-2 py-2 text-sm" @change="onChanged(true)">
...
// list hiển thị (client-side refine nếu cần)
const list = computed(() => {
  let arr = raw.value.slice()

  // (nếu BE chưa hỗ trợ sort/search thì vẫn có client fallback)
  ...
  switch (sortBy.value) {
    ...
    case 'lastLoginAt':
      arr.sort((a, b) => (new Date(a.lastLoginAt || 0).getTime()) - (new Date(b.lastLoginAt || 0).getTime())); break
    default:
...
function fallbackAvatar(u: User) {
  return `https://i.pravatar.cc/100?u=${encodeURIComponent(String(u.id))}` // chỉ dùng dev
}
function badgeClass(s: UserStatus) {
  switch (s) {
    ...
    case 'banned': return 'border-rose-200 bg-rose-50 text-rose-700'
    case 'pending_approval': return 'border-sky-200 bg-sky-50 text-sky-700'
  }
}
function statusText(s: UserStatus) {
  return s === 'active' ? 'Đang hoạt động'
    : s === 'locked' ? 'Khoá tạm'
    : s === 'banned' ? 'Cấm'
    : 'Chờ duyệt'
}

// debounce để tránh gọi API dồn dập khi gõ
...
// tự động fetch khi thay đổi filter chính
