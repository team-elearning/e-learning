<template>
  <div class="min-h-screen w-full overflow-x-hidden bg-slate-50">
    <main class="w-full mx-auto max-w-screen-2xl px-4 py-6 sm:px-6 md:px-10">
      ...
      <div v-else class="grid grid-cols-1 gap-3">
        ...
        >
          ...
          <div class="flex w-full sm:w-auto shrink-0 gap-2">
            <button class="flex-1 sm:flex-none rounded-xl border px-3 py-2 text-sm hover:bg-slate-50" @click="openFeedback(u.id)">
              Phản hồi
            </button>
          </div>
        </article>
      ...
      <div class="mt-6">
        <!-- Compact on mobile -->
        <div v-if="isCompact" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div class="text-sm text-slate-600">
            Tổng {{ total }} • Trang {{ page }} / {{ totalPages || 1 }}
          </div>
          <div class="flex items-center gap-2">
            <button class="rounded-xl border px-3 py-2 text-sm disabled:opacity-50" :disabled="page<=1" @click="go(page-1)">Trước</button>
            <span class="text-sm text-slate-600">Trang {{ page }} / {{ totalPages || 1 }}</span>
            <button class="rounded-xl border px-3 py-2 text-sm disabled:opacity-50" :disabled="page>=totalPages" @click="go(page+1)">Sau</button>
...
function updateCompact() { isCompact.value = window.innerWidth < 640 }
...
.select-base::-ms-expand{ display:none; } /* old Edge/IE */
.select-base:focus{
  outline: none;
  border-color: #7dd3fc; /* sky-300 */
  box-shadow: 0 0 0 2px rgba(2,132,199,.28);
}
.select-base:hover{ border-color:#cbd5e1; } /* slate-300 */
</style>
